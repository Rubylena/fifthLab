        ...
        <div className="absolute inset-0 flex items-center justify-center z-10">
          <div className="flex items-center justify-center pt-4 px-4 pb-20">
            ...
            >
              <div
                ref={modalRef}
                className="z-20 p-5 absolute top-0 inset-0 m-10 sm:m-20 align-bottom bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:align-middle "
              >
                <button
                  onClick={() => setOpen(false)}
                  type="button"
                  className="inline-flex gap-2 font-medium text-cyan-500 focus:outline-none sm:ml-3 sm:w-auto text-sm"
                >
                  <ArrowLeftIcon className="h-5 w-5 text-cyan-500" />
                  RESULTS
                </button>

                <div className="flex justify-between bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 mt-5">
                    ...
                    <div className="flex flex-col gap-3">
                      <div>
                        <p className="text-2xl font-bold">
                          {details?.[0]?.name?.title}{" "}
                          {details?.[0]?.name?.first} {details?.[0]?.name?.last}
                      ...
                      <div className="flex items-center gap-2 bg-pink-300 rounded-lg py-1 px-3">
                        <p>JOINED:</p>
                        <p>
